...
async def run_ingestion(file_path: str):
    ...
    if language == "unknown":
        logger.warning(
            f"Langage inconnu pour le fichier {file_path}, tentative avec 'python'"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="CLI pour l'écosystème JabbarRoot Analyzer."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Création de la sous-commande 'ingest'
    ingest_parser = subparsers.add_parser(
        "ingest", help="Lancer le pipeline d'ingestion sur un fichier."
    )
    ingest_parser.add_argument(
        "file", type=str, help="Le chemin vers le fichier à analyser."
    )
