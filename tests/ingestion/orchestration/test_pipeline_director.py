...
async def test_pipeline_director_stage_execution_order(mocker):
    ...
    final_context = await director.process(context.file_path, context.source_code, context.language)

    # Assert
    mock_stage1.execute.assert_called_once()
    mock_stage2.execute.assert_called_once()
    
    assert mock_stage2.execute.call_args[0][0].entities == [{'name': 'entity1', 'type': 'FUNCTION'}]
